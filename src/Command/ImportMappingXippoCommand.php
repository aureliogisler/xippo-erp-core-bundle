...
namespace App\XippoErp\CoreBundle\Command;
 ...
 * @final
 */
class ImportMappingXippoCommand extends DoctrineCommand
{
    /** @var string[] */
    private $bundles;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($metadata) {
            ...
            foreach ($metadata as $class) {
                $inflector = InflectorFactory::create(Language::ENGLISH)->build();
                $singularize = $inflector->singularize($class->name);
                if (is_array($singularize)) {
                    $singularize = $singularize[0];
                }

                $className   = $singularize;
